...
(function () {
    ...
    function loadRankings() {
        ...
        StockAPI.getRankings(date, state.currentMarket)
            ...
            .catch(function (err) {
                ...
                if (window.Toast) {
                    window.Toast.show('데이터를 불러올 수 없습니다', {
                        type: 'error',
                        duration: 0,
                        action: { label: '다시 시도', onClick: loadRankings }
                    });
                }
                console.error(err);
            });
    ...
    function saveMemo() {
        ...
        closeMemo();
        renderTable();
        if (window.Toast) window.Toast.show(text ? '메모가 저장되었습니다' : '메모가 비워졌습니다', { type: 'success' });
    }
    ...
    function saveTag() {
        ...
        closeTagEdit();
        renderTable();
        if (window.Toast) window.Toast.show('태그가 저장되었습니다', { type: 'success' });
    }
